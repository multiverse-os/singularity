...
func get(path string, name string, getxattrFunc getxattrFunc) ([]byte, error) {
	...
	for {
		...
		if err == syscall.ERANGE || read == size {
			...
			size <<= 1
			if size >= maxBufSize {
				return nil, &Error{attrName, path, name, syscall.EOVERFLOW}
			}
			continue
		}
		if err != nil {
			return nil, &Error{attrName, path, name, err}
		}
...
func list(path string, listxattrFunc listxattrFunc) ([]string, error) {
	attrName := prefix + "xattr.list"
	// find size.
	size, err := listxattrFunc(nil)
	if err != nil {
		return nil, &Error{attrName, path, "", err}
	}
	if size > 0 {
